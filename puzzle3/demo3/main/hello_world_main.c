 ...
 * @LastEditTime: 2023-11-24 03:32:27
 * @FilePath: \demo3\main\hello_world_main.c
...
static void ProcessFunc(char *buf,int len)
{
    ...
    fclose(f);
    //串口输出内容
    uart1_send((const char *)rev_buf,rev_len);
    ESP_LOGI(TAG,"%s\r\n",rev_buf);
...
static void https_test_task(void *pvParameters)
{
    ...
    https_get_request_using_crt_bundle();   //https请求
...
static void User_interaction_ask(void *pvParameters)
{
        // Configure a temporary buffer for the incoming data
    uint8_t *data = (uint8_t *) malloc(g_UART1_BUF_SIZE);

    while (1) {
        // Read data from the UART
        int len = uart_read_bytes(MY_UART_NUM, data, (g_UART1_BUF_SIZE - 1), 20 / portTICK_RATE_MS);
                // Write data back to the UART
        //uart_write_bytes(MY_UART_NUM, (const char *) data, len);
        if (len) {
            data[len] = '\0';
            if(strcmp((const char *)data,"fetch")==0)
            {
                //uart_write_bytes(MY_UART_NUM, (const char *) data, len);
                //任务通知
                if(xTaskNotifyGive(xNotifyTask) == pdPASS)
                {
                    ESP_LOGI(TAG, "任务通知成功,删除任务");
                    break;
                }else
                {
                    ESP_LOGE(TAG,"任务通知失败");
...
void app_main(void)
{
    ...
    uart1_init(115200);
    //用户交互任务
    xTaskCreate(&User_interaction_ask, "User_interaction_ask", 2048, NULL, 1, &xNotifyTask);

    //创建网络任务
    xTaskCreate(&https_test_task, "https_test_task", 8192, NULL, 5, &xNotifyTask);
